...
let autoIncome = 0;
let multiplier = 1;
let tickInterval = 1000;           // ms per income tick
...
const rarities = [
  ...
  // Easter egg (end of array so it won't be returned by fallback)
...
/* Elements (assumes these IDs exist in the HTML) */
...
function formatNumber(n) {
  // simple pretty formatting with commas; safe for large numbers in JS Number range
  if (typeof n !== 'number') return n;
  if (!isFinite(n)) return n.toString();
  if (Math.abs(n) >= 1e9) return n.toLocaleString(); // for big values
  return Math.floor(n).toLocaleString();
}

/* Rarity selection: use totalChance so provided chance numbers are respected */
function getRandomRarity() {
  const total = rarities.reduce((sum, r) => sum + (r.chance || 0), 0);
  ...
  // fallback to Common (safe default)
...
function renderShops() {
  ...
  currentItemSlots.forEach((slot, idx) => {
  ...
  currentPetSlots.forEach((slot, idx) => {
    const div = document.createElement('div');
    div.className = 'slot';
...
function updateDisplay() {
  ...
  incomeEl.textContent = formatNumber(autoIncome * multiplier);
  tickIntervalDisplayEl.textContent = (tickInterval / 1000).toFixed(2) + 's';
  tickBoostStatusEl.textContent = tickInterval < 1000 ? 'Fast' : 'Normal';
  boostStatusEl.textContent = (boostActiveUntil && Date.now() < boostActiveUntil) ? 'Active' : 'None';
}
...
// decrement cooldown timers once per second
setInterval(() => {
...
document.getElementById('buyBoost').onclick = () => {
  ...
  money -= 1000;
  multiplier *= 2;
  boostActiveUntil = Date.now() + 5 * 60 * 1000; // 5 minutes
  boostCooldown = 15 * 60; // cooldown seconds
  boostTimer = 5 * 60;
  showNotification('2x Income Boost activated for 5 minutes!');
  updateDisplay();
};

document.getElementById('buyTickBoost').onclick = () => {
  if (money < tickBoostCost) return alert('Not enough money');
  money -= tickBoostCost;
  tickBoostCost += 1000000; // increase cost each purchase
  tickInterval = Math.max(100, tickInterval - 100); // speed up tick, min 100ms
  showNotification('Tick speed boosted! (tick interval decreased)');
  updateDisplay();
  updateBoostButtonsText();
};
...
document.getElementById('runAdmin').onclick = () => {
  ...
  if (cmd === 'givemoney' || cmd === 'addmoney') {
    ...
    const name = itemNames[Math.floor(Math.random() * itemNames.length)];
    const slot = { name, rarityName: r.name, bonus: r.bonus, cost: r.cost, color: r.color, bought: true };
    itemsOwned.push({ name: slot.name, rarityName: slot.rarityName, bonus: slot.bonus, color: slot.color });
    autoIncome += r.bonus;
    renderItemsOwned();
...
/* Shop timers and auto refresh (every second) */
setInterval(() => {
  shopCountdown--;
  petShopCountdown--;
